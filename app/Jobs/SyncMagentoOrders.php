...
class SyncMagentoOrders implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(): void
    {
        foreach ($this->tenantIds as $tenantId) {
            try {
                if (!(new CheckTokenUtils())->execute($tenantId, IntegrationEnum::MAGENTO)) {
                    continue;
                }
                ...
                Log::error('Error during Magento orders synchronization for tenant ' . $tenantId . ': ' . $e->getMessage());
            }
        }
    }
}
