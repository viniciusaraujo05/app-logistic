...
class SyncWortenOrders implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(): void
    {
        foreach ($this->tenantIds as $tenantId) {
            try {
                if (!(new CheckTokenUtils())->execute($tenantId, IntegrationEnum::WORTEN)) {
                    continue;
                }
                ...
                Log::error('Error during Worten orders synchronization for tenant ' . $tenantId . ': ' . $e->getMessage());
            }
        }
    }
}
